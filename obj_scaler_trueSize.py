...
class ScalingGUI:
    def __init__(self):
        ...
        # Scale divisors for calculating target Y dimension
        # Formula: target_y = (current_y * 1000) / divisor
        self.scale_divisors = {
            'True Scale': None,  # Special case: multiply by 1000
            'G': 22.5,
            'O': 48,
            'S': 64,
            'HO': 87,
            'N': 160,
            '6in': None,  # Keep original fixed value
            '4in': None,  # Keep original fixed value
            '3in': None   # Keep original fixed value
        }
        
        # Fixed values for inch scales (unchanged)
        self.fixed_scales = {
            '6in': 150,
            '4in': 100,
            '3in': 75
    ...
    def setup_ui(self):
        ...
        scales_frame = ttk.LabelFrame(self.root, text="Standard Scales (Y dimension calculated)", padding="10")
        scales_frame.pack(fill=tk.X, padx=10, pady=5)
        
        # True Scale button (first row)
        true_scale_row = ttk.Frame(scales_frame)
        true_scale_row.pack(fill=tk.X, pady=2)
        ttk.Button(true_scale_row, text="True Scale (Y × 1000)", command=lambda: self.create_scale('True Scale')).pack(side=tk.LEFT, padx=2)
        
        # First row of scale buttons
        scale_row1 = ttk.Frame(scales_frame)
        scale_row1.pack(fill=tk.X, pady=2)
        ttk.Button(scale_row1, text="G Scale", command=lambda: self.create_scale('G')).pack(side=tk.LEFT, padx=2)
        ttk.Button(scale_row1, text="O Scale", command=lambda: self.create_scale('O')).pack(side=tk.LEFT, padx=2)
        ttk.Button(scale_row1, text="S Scale", command=lambda: self.create_scale('S')).pack(side=tk.LEFT, padx=2)
        ttk.Button(scale_row1, text="HO Scale", command=lambda: self.create_scale('HO')).pack(side=tk.LEFT, padx=2)
        
        # Second row of scale buttons
        scale_row2 = ttk.Frame(scales_frame)
        scale_row2.pack(fill=tk.X, pady=2)
        ttk.Button(scale_row2, text="N Scale", command=lambda: self.create_scale('N')).pack(side=tk.LEFT, padx=2)
    ...
    def calculate_target_y(self, scale_name):
        """Calculate target Y dimension based on scale name and current Y dimension.
        
        Args:
            scale_name: Name of the scale ('True Scale', 'G', 'O', 'S', 'HO', 'N', etc.)
            
        Returns:
            Target Y dimension in mm, or None if scale not found or file not loaded
        """
        if self.scaler.extents is None:
            return None
        
        current_y = self.scaler.extents[1]  # Current Y dimension
        
        if scale_name == 'True Scale':
            # True Scale: multiply Y by 1000
            return current_y * 1000
        elif scale_name in self.scale_divisors:
            divisor = self.scale_divisors[scale_name]
            if divisor is not None:
                # Formula: (Y * 1000) / divisor
                return (current_y * 1000) / divisor
        elif scale_name in self.fixed_scales:
            # Fixed scales (6in, 4in, 3in) - return fixed value
            return self.fixed_scales[scale_name]
    ...
    def is_scaled_file(self, file_path):
        ...
        all_scales = list(self.scale_divisors.keys()) + list(self.fixed_scales.keys())
    ...
    def create_scale_for_file(self, file_path, scale_name, suppress_dialogs=False):
        ...
        if scale_name not in self.scale_divisors and scale_name not in self.fixed_scales:
            if not suppress_dialogs:
                messagebox.showerror("Error", f"Unknown scale: {scale_name}")
            return False
        
        # Calculate target Y dimension
        target_y = self.calculate_target_y(scale_name)
        if target_y is None:
            if not suppress_dialogs:
                messagebox.showerror("Error", f"Could not calculate target Y for scale: {scale_name}")
            return False
    ...
    def create_scale(self, scale_name):
        ...
        if run_all:
            ...
            # Calculate target Y for display (use first file as example)
            example_target_y = None
            if original_files:
                temp_path = self.scaler.obj_path
                self.scaler.obj_path = original_files[0]
                self.scaler.find_mtl_file()
                if self.scaler.parse_obj_file(suppress_dialogs=True):
                    example_target_y = self.calculate_target_y(scale_name)
                self.scaler.obj_path = temp_path
            
            target_y_display = f"{example_target_y:.2f}mm" if example_target_y else "calculated"
            
            # Clear log and start batch processing
            self.clear_log()
            self.log_message(f"Starting batch processing: Scale {scale_name} (target Y: {target_y_display})")
            ...
            messagebox.showinfo("Batch Processing Complete", 
                              ...
                              f"Scale: {scale_name} (target Y: {target_y_display})")
        else:
        ...
        if run_all:
            ...
            for file_idx, file_path in enumerate(original_files, 1):
                ...
                if not self.scaler.parse_obj_file(suppress_dialogs=True):
                    ...
                    files_failed += 1
                    all_scales_count = len(self.scale_divisors) + len(self.fixed_scales)
                    total_failed += all_scales_count
                    continue
